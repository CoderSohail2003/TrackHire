...
export default function PreviewModal({ job, onClose }) {
  ...
  // Also close the modal if the user hits the "Escape" key
  useEffect(() => {
  ...
  return (
    <div 
      className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 overflow-y-auto animate-in fade-in duration-200"
      onClick={onClose}
    >
      <div 
        className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden my-8 animate-in zoom-in-95 duration-200"
        onClick={(e) => e.stopPropagation()}
      >
         {/* Futuristic Header with Gradient and Glassmorphism */}
         <div className="relative bg-gradient-to-r from-indigo-600 to-blue-500 p-8 text-white">
            <button 
              onClick={onClose} 
